...
import { BookOpen, Plus, Search, Copy, Check, Trash2, Edit3, X, Tag, Save } from "lucide-react";
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ scale: 0.96, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={onClose} className="p-1.5 rounded-lg transition-colors" style={{ color: "var(--md-on-surface-var)" }}>
            <X className="w-4 h-4" />
          </button>
        </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5 mb-2">
              {tags.map(t => (
                <span key={t} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs"
                  ...
                  {t}
                  <button onClick={() => setTags(prev => prev.filter(x => x !== t))} className="hover:opacity-70"><X className="w-2.5 h-2.5" /></button>
                </span>
              ))}
            </div>
            <div className="flex gap-2">
              <input value={tagInput} onChange={e => setTagInput(e.target.value)}
                ...
                className="flex-1 px-3.5 py-2 rounded-xl text-xs"
                style={{ background: "var(--md-surface-2)", border: "1px solid var(--md-outline)", color: "var(--md-on-surface)" }} />
              <button onClick={addTag} className="px-3 py-2 rounded-xl text-xs font-medium transition-colors"
                style={{ background: "var(--md-primary-container)", color: "var(--md-on-primary-cont)" }}>
                Add
              </button>
            </div>
        ...
        <div className="flex gap-3 mt-6">
          ...
          <button onClick={handleSubmit} className="flex-1 py-2.5 rounded-xl text-sm font-medium flex items-center justify-center gap-2 transition-opacity hover:opacity-90"
    ...
    <DashboardLayout>
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
        ...
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-xl font-semibold" style={{ color: "var(--md-on-surface)" }}>Prompt Library</h1>
            <p className="text-sm mt-0.5" style={{ color: "var(--md-on-surface-var)" }}>
              Save, organize and reuse your best prompts. {prompts.length} prompts saved.
            </p>
          </div>
        ...
        <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <h3 className="font-medium text-sm" style={{ color: "var(--md-on-surface)" }}>{prompt.title}</h3>
                      ...
                      <div className="flex items-center gap-3 mt-0.5">
                        {prompt.tags.map(t => (
                          <span key={t} className="text-[10px]" style={{ color: "var(--md-on-surface-var)" }}>#{t}</span>
                        ))}
                        <span className="text-[10px]" style={{ color: "var(--md-on-surface-var)" }}>Updated {prompt.updatedAt}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  {expanded === prompt.id && (
                    <motion.div initial={{ height: 0 }} animate={{ height: "auto" }} exit={{ height: 0 }}
                      ...
                      <div className="px-5 pb-4" style={{ borderTop: "1px solid var(--md-outline-var)" }}>
                        <pre className="text-xs font-mono whitespace-pre-wrap leading-relaxed mt-3 p-4 rounded-xl"
                          style={{ background: "var(--md-surface-2)", color: "var(--md-on-surface)", overflowX: "auto" }}>
                          {prompt.content}
                        </pre>
                        <p className="text-[10px] mt-2" style={{ color: "var(--md-on-surface-var)" }}>
          ...
          {filtered.length === 0 && (
            <div className="text-center py-16">
              ...
              <p className="text-sm font-medium mb-1" style={{ color: "var(--md-on-surface)" }}>No prompts found</p>
